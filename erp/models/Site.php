...
//						echo 'hello'; exit();
    ...
    public function getAPaging($id=null,$ware=null,$created=null,$biller=null,$Sdate=null,$Edate=null,$wh=null)
    {
        // echo $Sdate;exit();
        // $this->erp->print_arrays($wh);
        // echo $id;exit();

        $this->db->select("purchases.supplier_id,companies.name ,SUM(IFNULL(grand_total, 0)) AS grand_total,	SUM(IFNULL(paid, 0)) AS paid,SUM(IFNULL(grand_total - paid, 0)) AS balance,	COUNT(erp_purchases.id) AS ap_number, purchases.date")
            ->from('purchases')
            ->join('companies','purchases.supplier_id = companies.id', 'inner')
            ->where('payment_status !=', 'Returned')
            ->where('payment_status !=', 'paid')
            ...
            ->where('(grand_total-paid) <> ', 0);
        if($ware){
            $this->db->where('erp_purchases.warehouse_id',$ware);
        }
        if($created){
            $this->db->where('erp_purchases.created_by',$created);
        }
        if($biller){
        ...
        if($wh){
            $this->db->where_in('purchases.warehouse_id',$wh);
        }
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            return $q->row();
    ...
    public function getAPaging2($id)
    {
        // echo $Sdate;exit();
        // $this->erp->print_arrays($wh);
        // echo $id;exit();

        $this->db->select("purchases.supplier_id,companies.name ,SUM(IFNULL(grand_total, 0)) AS grand_total,	SUM(IFNULL(paid, 0)) AS paid,SUM(IFNULL(grand_total - paid, 0)) AS balance,	COUNT(erp_purchases.id) AS ap_number")
            ->from('purchases')
            ->join('companies','purchases.supplier_id = companies.id', 'inner')
            ->where('payment_status !=', 'Returned')
            ->where('payment_status !=', 'paid')
    ...
    public function getAPaging3($id)
    {
        $this->db->select("purchases.supplier_id,companies.name, 
			SUM(
    ...
    public function getAPaging4($id)
    {
        $this->db->select("purchases.supplier_id,companies.name ,SUM(IFNULL(grand_total, 0)) AS grand_total,	SUM(IFNULL(paid, 0)) AS paid,SUM(IFNULL(grand_total - paid, 0)) AS balance,	COUNT(erp_purchases.id) AS ap_number, purchases.date")
            ->from('purchases')
            ->join('companies','purchases.supplier_id = companies.id', 'inner')
    ...
    public function getAPaging5($id)
    {
        $this->db->select("purchases.supplier_id,companies.name ,SUM(IFNULL(grand_total, 0)) AS grand_total,	SUM(IFNULL(paid, 0)) AS paid,SUM(IFNULL(grand_total - paid, 0)) AS balance,	COUNT(erp_purchases.id) AS ap_number, purchases.date")
            ->from('purchases')
            ->join('companies','purchases.supplier_id = companies.id', 'inner')
